        ...
        for (int i = 1; i <= 9; i++)
        {
            for (int j = 1; j <= 9; j++)
            {
                ...
                if ((value_just_read >= 1) && (value_just_read <= 9))
                {
                    ...
                    solution[(value_just_read) + (81*(i-1)) + (9*(j-1))] = 1;
                    add_constraint(lp, solution, EQ, 1);
...
// The ILP formulation is solved using the API for Lpsolve
// Pay attention to how the solution is interpretted...
void solve_the_puzzle_all()
{
    
    int ret = 0;
    int counter_val = 0;
    bool flag = false; //solution exists
    
    while (0 == (ret = solve(lp)))
    {
        counter_val++;
        flag = true;
        ...
        cout << endl <<"Solution # "<< counter_val << endl;
...
int main (int argc, char* argv[])
{
    ...
    /// use solve_the _puzzle_all for all solutions otherwise use the above function call
   // solve_the_puzzle_all();
